...
function ExerciseModal({ exercise, dayDate, visible, onClose }) {
  ...
  React.useEffect(() => {
    ...
    // fetch follows the redirect; r.url is the final direct CDN image URL
    fetch(`https://source.unsplash.com/800x500/?${encodeURIComponent(query)}`)
      .then(r => {
        if (r.url && r.url.includes('images.unsplash.com')) {
          setImgUrl(r.url);
        }
        setImgLoading(false);
      })
              ...
              {!imgLoading && imageUrl && !imgError ? (
                ...
                <LinearGradient colors={[cat.grad[0]+'44','#0A0A0F']} style={{ height:180, alignItems:'center', justifyContent:'center', padding:20 }}>
                  ...
                  <View style={{ marginTop:12, backgroundColor:'#00000066', borderRadius:10, paddingHorizontal:14, paddingVertical:8 }}>
                    <Text style={{ color:C.muted, fontSize:10, textAlign:'center' }}>📸 Cargando foto del ejercicio...</Text>
                  </View>
                </LinearGradient>
              ) : null}
            </View>
